...
function FrameHandler(stompServer) {
  this.CONNECT = function (socket, frame) {
    ...
    var parts = [0, 0];
    if (rawHeartBeat) {
      parts = rawHeartBeat.split(",").map(function (x) {
        return Number(x);
      });
    }
    var agreedHeartBeat = 0;
    if (stompServer.conf.heartBeatConfig.client > 0) {
      agreedHeartBeat = Math.max(stompServer.conf.heartBeatConfig.client, parts[0]);
    }

    if (stompServer.onClientConnected(socket, {
        heartBeat: [agreedHeartBeat, parts[1]],
        headers: frame.headers
      })) {
      rawHeartBeat = stompServer.conf.heartBeatConfig.server + "," + stompServer.conf.heartBeatConfig.client;
